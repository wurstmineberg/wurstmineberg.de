        ...
        html {
            ...
            body {
                            ...
                            @if let Some(me) = me {
                                ...
                                li {
                                    li(class = "navbar-user-notloggedin") {
                                        a(href = uri!(crate::auth::discord_login(Some(uri))), title = "You are not logged in.") {
                ...
                p(class = "muted text-center") {
                    ...
                    : crate::YEAR_OF_LAST_COMMIT.to_string();
                }
        ...
        div(class = "panel panel-default") {
            ...
            div(class = "panel-body") {
                ...
                p {
                    ...
                    : crate::wiki::link(db_pool, "modded-world", "wiki", "modded world").await?;
                    : ".";
                }
                p {
...
async fn flask_proxy_get(config: &State<Config>, http_client: &State<reqwest::Client>, proxy_http_client: &State<ProxyHttpClient>, me: Option<DiscordUser>, origin: Origin<'_>, headers: Headers, path: Segments<'_, Path>) -> Result<FlaskProxyResponse, FlaskProxyError> {
    ...
    match response.status() {
        ...
        _ => night_report(config, http_client, "/dev/gharch/flaskProxy", Some(&format!("GET {url} forwarded to Flask (response status: {})", response.status())), 0.0, 0.1).await.map_err(FlaskProxyError::NightReport)?,
    }
...
async fn flask_proxy_post(config: &State<Config>, http_client: &State<reqwest::Client>, proxy_http_client: &State<ProxyHttpClient>, me: Option<DiscordUser>, origin: Origin<'_>, headers: Headers, path: Segments<'_, Path>, data: Vec<u8>) -> Result<FlaskProxyResponse, FlaskProxyError> {
    ...
    match response.status() {
        reqwest::StatusCode::NOT_FOUND => {}
        reqwest::StatusCode::INTERNAL_SERVER_ERROR => return Err(FlaskProxyError::InternalServerError(response.text().await?)),
        _ => night_report(config, http_client, "/dev/gharch/flaskProxy", Some(&format!("POST {url} forwarded to Flask (response status: {})", response.status())), 0.0, 0.1).await.map_err(FlaskProxyError::NightReport)?,
    }
    Ok(FlaskProxyResponse::Proxied(Response(response)))
}
...
async fn internal_server_error(request: &Request<'_>) -> RawHtml<String> {
    ...
    let me = request.guard::<User>().await.succeeded();
    let uri = request.guard::<Origin<'_>>().await.succeeded().unwrap_or_else(|| Origin(uri!(index)));
    let is_reported = night_report(config, http_client, "/dev/gharch/webError", Some("internal server error"), 29.0, 1.0).await.is_ok();
...
async fn fallback_catcher(status: Status, request: &Request<'_>) -> RawHtml<String> {
    ...
    let me = request.guard::<User>().await.succeeded();
    let uri = request.guard::<Origin<'_>>().await.succeeded().unwrap_or_else(|| Origin(uri!(index)));
    let is_reported = night_report(config, http_client, "/dev/gharch/webError", Some("responding with unexpected HTTP status code"), 29.0, 1.0).await.is_ok();
...
pub(crate) async fn rocket(config: Config, discord_ctx: RwFuture<DiscordCtx>, http_client: reqwest::Client, proxy_http_client: reqwest::Client) -> Result<Rocket<rocket::Ignite>, crate::Error> {
    Ok(
        ...
        .mount("/", rocket::routes![
            ...
            crate::api::world_level,
            crate::api::world_level_json,
